...
static void showMenu() {
    std::cout << "\n========== LEADERBOARD ==========\n";
    std::cout << " 1. Add Team\n";
    std::cout << " 2. Record Mission (update score + missions)\n";
    std::cout << " 3. Delete Team\n";
    std::cout << " 4. Sort by Score & Display\n";
    std::cout << " 5. Sort by Missions & Display\n";
    std::cout << " 6. Display Unsorted\n";
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            if (lb.recordMission(id, score, missions, error)) {
                std::cout << "  Team record updated.\n";
            } else {
                std::cout << "  Update failed: " << error << "\n";
            }
        } else if (choice == 3) {
            int id = readInt("  Team ID to delete: ");
            if (lb.deleteTeam(id, error)) {
                std::cout << "  Team deleted.\n";
            } else {
                std::cout << "  Delete failed: " << error << "\n";
            }
        } else if (choice == 4) {
            lb.sortLeaderboard(0);
            lb.displayTeams();
        } else if (choice == 5) {
            lb.sortLeaderboard(1);
            lb.displayTeams();
        } else if (choice == 6) {
            lb.displayTeams();
        } else if (choice == 7) {
            int id = readInt("  Team ID to search: ");
            Team t;
            ...
            char filename[256];
            readName(filename, 256, "  Filename to load: ");
            if (lb.loadTeams(filename, error)) {
